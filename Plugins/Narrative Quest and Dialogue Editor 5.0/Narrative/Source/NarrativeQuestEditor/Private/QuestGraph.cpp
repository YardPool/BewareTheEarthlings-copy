				...
				for (auto& LinkedPin : Pin->LinkedTo)
				{
					if (UQuestGraphNode_Action* To = Cast<UQuestGraphNode_Action>(LinkedPin->GetOwningNode()))
					{
						for (auto& State : Quest->States)
						{
							...
							if (State->CompletionType == EStateMachineCompletionType::NotAccepted)
							{
...
{
	TSubclassOf<UQuestState> StateClass = GetDefault<UQuestEditorSettings>()->DefaultQuestState;
	...
	{
		State->CompletionType = EStateMachineCompletionType::Rejected;
	}
	else if (Cast<UQuestGraphNode_Success>(Node))
	{
		State->CompletionType = EStateMachineCompletionType::Accepted;
	}
...
{
	TSubclassOf<UQuestBranch> BranchClass = GetDefault<UQuestEditorSettings>()->DefaultQuestBranch;
