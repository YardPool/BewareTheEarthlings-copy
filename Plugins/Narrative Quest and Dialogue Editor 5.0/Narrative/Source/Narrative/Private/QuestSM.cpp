...
UQuestState* UQuestBranch::TryBranch(const FString& Input, bool& bOutMadeProgress)
{
	//We shouldnt have a quest state that doesn't have a quest
	check(GetOwningQuest());

	if (UNarrativeComponent* NarrativeComp = GetOwningNarrativeComp())
	{
...
FStateMachineResult UQuestState::RunState(bool& bOutMadeProgress, const FString& Input)
{
	...
	if (UNarrativeComponent* NarrativeComp = GetOwningNarrativeComp())
	{
		...
		{
			DestinationState = Branch->TryBranch(Input, bOutMadeProgress);
...
{
	if (UQuest* OwningQuest = GetOwningQuest())
	{
		return OwningQuest->GetOwningNarrativeComponent();
	}
...
{
	check(OwningBranch && NarrativeComp);
